...
export function ParticipantItem({ participant, isHost, onMute, onKick }: ParticipantItemProps) {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 flex-1 min-w-0">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-medium text-card-foreground truncate">
            {participant.name}
            {participant.isHost && <span className="text-xs text-muted-foreground ml-1">(Host)</span>}
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-1">
        ...
        {/* Host-only actions */}
        {isHost && participant.id !== "1" && showActions && (
